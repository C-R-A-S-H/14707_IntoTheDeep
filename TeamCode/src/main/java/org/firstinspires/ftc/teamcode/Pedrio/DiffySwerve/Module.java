...
public class Module {
    ...
    final double turn_kP = 0.1;
    final double drive_kP = 0.1;
    final private PIDController turnPid = new PIDController(turn_kP,0,0);

    final private PIDFController drivePid = new PIDFController(drive_kP,0,0,0);//getMotorVelocityF(MAX_RPM / 60 * ENCODER_TICKS_PER_REV));


    final private PIDController motor1PID = new PIDController(0.1,0,0);//these should be the same
    final private PIDController motor2PID = new PIDController(0.1,0,0);
    ...
    public void moveTo(double velocity, double targetAngle){
        ...
        if (Math.abs(oppAngleFromTarget) < 0.01){//margin of allowed error
            velocity = -velocity;
        }else if (Math.abs(angleFromTarget) > 0.01){
            //velocity = 0;
        }
    ...
    public void angleTest(double targetAngle){
        oppAngle = angleWrap(getModuleHeading() + 180);
